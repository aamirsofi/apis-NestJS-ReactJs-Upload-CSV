...
const CsvUploader: React.FC<CsvUploaderProps> = ({
  ...
  const handleConfirmUpload = useCallback(
    async (options?: DuplicateDetectionOptions) => {
      ...
      try {
        ...
        let successMessage = `CSV file "${
          previewFile.name
        }" uploaded successfully! ${result.totalRows} row${
          result.totalRows !== 1 ? "s" : ""
        } imported.`;

        if (result.duplicateCount && result.duplicateCount > 0) {
          successMessage += ` ${result.duplicateCount} duplicate${
    ...
    <>
      ...
      <div
        className={`card-modern${
          darkMode ? "-dark" : ""
        } rounded-2xl p-8 transition-smooth hover-lift`}
      >
        <div
          {...getRootProps()}
          className={`
        ...
        >
          ...
          <div className="flex flex-col items-center">
            ...
            <div
              className={`mb-6 p-6 rounded-2xl transition-smooth hover:scale-110 ${
                darkMode
