...
const AuditLogs: React.FC<AuditLogsProps> = ({ darkMode = false }) => {
  ...
  const [pageSize, setPageSize] = useState<number>(25); // Reduced default from 50
  ...
  const hasActiveFilters =
    actionFilter !== "all" || uploadIdFilter.trim() || startDate || endDate;
      ...
      <div className={`card-modern${darkMode ? "-dark" : ""} rounded-2xl p-8`}>
        <div className="flex flex-col justify-center items-center py-12">
          <div className="relative">
            ...
            ></div>
            <div
              className={`absolute inset-0 animate-ping rounded-full h-12 w-12 ${
                darkMode
  ...
  return (
    <div
      className={`card-modern${
        darkMode ? "-dark" : ""
      } rounded-2xl p-8 transition-smooth`}
    >
      {/* Header */}
      <div className="mb-6">
        <h2
      ...
      {logs && logs.logs.length > 0 ? (
        <>
          <div className="overflow-x-auto rounded-xl border overflow-hidden mb-6 max-h-[600px] overflow-y-auto">
            <table
                  ...
                  >
                    ...
                    >
                      {log.fileName ? (
                        ...
                        Array.isArray(log.details.fileNames) &&
                        log.details.fileNames.length > 0 ? (
                        <div className="flex flex-col">
                          ...
                          {log.details.fileNames.length > 1 && (
                            ...
                            >
                              +{log.details.fileNames.length - 1} more file
                              {log.details.fileNames.length - 1 !== 1
                                ? "s"
                                : ""}
                            </span>
                          )}
                    ...
                    >
                      {log.uploadId ? (
                        <span className="text-xs" title={log.uploadId}>
                          {log.uploadId.substring(0, 8)}...
                        </span>
                      ) : (
                        <span className="opacity-50">-</span>
                      )}
            ...
            >
              <div className="flex items-center gap-2 text-sm">
                <span>
                  ...
                  {Math.min(currentPage * pageSize, logs.total)} of {logs.total}{" "}
              ...
              <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1">
                  {Array.from(
                    { length: Math.min(7, logs.totalPages) },
                    (_, i) => {
                      let pageNum: number;
                ...
                <button
                  onClick={() =>
                    setCurrentPage((prev) =>
                      Math.min(logs.totalPages, prev + 1)
                    )
                  }
